...
- 3D zone rendering with textures, character/NPC models, and animations
- Basic movement (WASD) with collision detection
- Chat (all channels), basic targeting, and auto-attack
- Inventory display with item tooltips
- Most UI windows render and respond to input

**What doesn't work or is incomplete:**
- No character creation (requires existing account/character)
- Combat abilities, tradeskills, and most skills are display-only
- Spell effects are visual placeholders; server calculates all outcomes
- NPC dialog/quest text is received but not displayed in windows
- Swimming, levitation, jumping have minimal or no implementation
- Zone transitions sometimes have coordinate issues

**Architecture note:** This is a thin client that delegates most game logic to the server. Combat damage, spell effects, skill checks, and item transactions are all server-calculated. The client primarily handles rendering, input, and state tracking.
...
| Zone transitions | ⚠️ | Some coordinate issues |
| Zone points | ✅ | |
| Boats/transport | ❌ | |
...
| Auto-attack (melee) | ✅ | Toggle works, damage is server-calculated |
| Damage messages | ✅ | Display only |
| HP tracking | ✅ | |
| Consider | ✅ | |
| Death/corpse | ✅ | |
| Corpse looting | ✅ | Items and money |
| Spell casting | ⚠️ | Commands work, effects are server-side |
| Combat abilities | ❌ | Kick, bash, taunt, etc. |
| Ranged combat | ❌ | Bows, throwing |
| Pet commands | ✅ | Full pet window with commands |
| Bind wound | ❌ | |

### Spells — 45%
| Feature | Status | Notes |
|---------|--------|-------|
| Spell book | ⚠️ | Window exists, browsing incomplete |
| Memorize spells | ⚠️ | /mem, /forget - framework only |
| Cast spells | ⚠️ | Commands work, server calculates effects |
| Spell effects | ❌ | Visual effects not implemented |
| Buff window | ⚠️ | Window exists, display-only |
...
| Spell book | ⚠️ | Partial - /spellbook command exists |
| Spell bar | ⚠️ | Keys 1-8, /gems |
| Hotbar | ❌ | |
| Group window | ✅ | HP/mana bars, G key |
| Pet window | ✅ | Commands and status |
| Compass | ❌ | |
| Consider window | ❌ | |
| Options | ❌ | |
| Trade window | ✅ | Full player trading |
| Bank window | ✅ | Slots, shared bank, currency |
| Merchant window | ✅ | Buy/sell with pricing |
...
| Merchant buy/sell | ✅ | Full buy/sell with pricing |
| Banker | ✅ | Slots, shared bank, currency |
| Quest/NPC text | ✅ | Displayed in chat window |

### Skills — 30%
| Feature | Status | Notes |
|---------|--------|-------|
| Skill display | ✅ | Window shows all skills with values |
| Skill activation | ⚠️ | Framework exists, server validates |
| Skill use | ❌ | Most skills not functional |
| Tradeskills | ⚠️ | Container window, combines work |
| Forage | ❌ | |
| Fishing | ❌ | |
| Tracking | ❌ | |
| Sense heading | ❌ | |
