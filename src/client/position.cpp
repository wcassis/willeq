...
// Helper template for ValueWithin
template<typename T>
inline bool ValueWithin(T value, T min, T max) {
...
bool IsWithinAxisAlignedBox(const glm::vec3 &position, const glm::vec3 &minimum, const glm::vec3 &maximum) {
	...
	const bool x_check = ValueWithin(position.x, min.x, max.x);
	const bool y_check = ValueWithin(position.y, min.y, max.y);
	const bool z_check = ValueWithin(position.z, min.z, max.z);
...
bool IsWithinSquare(glm::vec4 center, uint32 area, glm::vec4 position) {
	auto l = std::abs(std::sqrt(static_cast<float>(area)));
	if (l <= 0) {
		return false;
	}
